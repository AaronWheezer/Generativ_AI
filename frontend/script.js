document.addEventListener('DOMContentLoaded', () => {
  ...
  // Generate or reuse a per-browser persistent sessionId
  ...
  function addMessage(text, sender) {
    ...
    const now = new Date();
    timestampDiv.innerText = `${now.getHours()}:${now
      .getMinutes()
      .toString()
      .padStart(2, '0')}`;
  ...
  async function handleBotResponse(userMessage) {
    ...
    try {
      ...
      addMessage(
        'Sorry, ik kan momenteel geen verbinding maken met de server. Controleer of de backend draait.',
        'bot'
      );
    }
  }
