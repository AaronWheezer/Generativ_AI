...
// Serve frontend static files from ../frontend so visiting / loads the UI
  ...
  for (const group of SYNONYMS) {
    if (group.some((term) => lower.includes(term))) {
      for (const term of group)
        expansions.add(lower.replace(/\b(?:" + group.join('|') + ")\b/, term));
      ...
      async (err, rows) => {
        ...
        for (const r of rows) {
          ...
          if (!Array.isArray(embArr) || embArr.length === 0) {
            ...
            if (embArr.length)
              db.run('UPDATE verkeersregels SET embedding=? WHERE id=?', [
                JSON.stringify(embArr),
                r.id,
              ]);
          }
...
function determinePriority(description, suspectKnown) {
  ...
  if (
    /overvallen|geweld|mes|wapen|inbraak|bedreigd|brand|aanranding|vernieling/.test(
      d
    )
  )
    return 'HOOG';
  if (/diefstal|gestolen|fiets|gsm|telefoon|portemonnee|fraude|scam/.test(d))
    return suspectKnown ? 'MIDDEN' : 'LAAG';
  return 'MIDDEN';
}

function truncate(t, max = 180) {
...
function buildSummary(f) {
  return [
    ...
    `Dader bekend: ${
      f.suspectKnown === true ? 'Ja' : f.suspectKnown === false ? 'Nee' : '-'
    }`,
    `Details dader: ${truncate(f.suspectDetails, 120) || '-'}`,
    `Bewijs/Getuigen: ${truncate(f.evidence, 120) || '-'}`,
    f.additional?.length
      ? `Extra: ${truncate(f.additional.join(' | '), 160)}`
      : '',
  ]
    .filter(Boolean)
    .join(' \n');
...
async function createDossier(sessionId) {
  ...
  return new Promise((resolve) => {
    const stmt = db.prepare(
      'INSERT INTO dossiers (datum, beschrijving, samenvatting, prioriteit) VALUES (?, ?, ?, ?)'
    );
    stmt.run(
      f.datetime || new Date().toISOString(),
      f.description || '(geen beschrijving)',
      summary,
      priority,
      function (err) {
        if (err) {
          ...
          resolve(
            `Proces-verbaal opgeslagen (#${id}). Prioriteit: ${priority}. Wilt u nog iets anders doen?`
          );
        }
      }
    );
...
async function getAIResponse(sessionId, message) {
  ...
  const incidentRegex =
    /(aangifte|bestolen|gestolen|diefstal|inbraak|overvallen|bedreigd|mishandeld|geweld|aanranding|vernield|vernieling|fraude|scam|fiets|gsm|telefoon|portemonnee)/;
  const isIncidentTrigger = incidentRegex.test(lower);
  const isQuestion =
    /\?|wat|hoe|mag|welke|wanneer/.test(lower) && !isIncidentTrigger;
  ...
  if (st.mode === 'idle' && st.report.step === 'start') {
    if (isIncidentTrigger && !isQuestion) {
      ...
      return {
        reply: 'Gaat dit over een aangifte. Wat is uw naam?',
        rag: { used: false },
      };
    }
    if (/aangifte|incident|melding/.test(lower)) {
      ...
      st.report.step = 'ask_name';
      return {
        reply: 'We starten een aangifte. Wat is uw naam?',
        rag: { used: false },
      };
    }
    if (/vraag|verkeer|wegcode/.test(lower) || isQuestion) {
      ...
      return {
        reply:
          'Gaat dit over een aangifte die u wilt maken of een algemene vraag?',
        rag: { used: false },
      };
    }
  }
  ...
  if (st.mode === 'report') {
    ...
    switch (st.report.step) {
      case 'ask_name':
        ...
        return {
          reply: 'Dank u. Beschrijf kort wat er is gebeurd.',
          rag: { used: false },
        };
      case 'ask_description':
        ...
        return {
          reply: 'Is er een dader bekend? (ja/nee)',
          rag: { used: false },
        };
      case 'ask_suspect_known': {
        if (/ja|wel|bekend/.test(lower)) {
          f.suspectKnown = true;
          st.report.step = 'ask_suspect_details';
          return {
            reply: 'Kunt u de dader beschrijven (naam, kenmerken)?',
            rag: { used: false },
          };
        }
        if (/nee|onbekend|geen/.test(lower)) {
          f.suspectKnown = false;
          st.report.step = 'ask_location';
          return { reply: 'Waar is dit gebeurd?', rag: { used: false } };
        }
        return {
          reply: 'Gelieve "ja" of "nee" te antwoorden. Is de dader bekend?',
          rag: { used: false },
        };
      }
      ...
      case 'ask_location':
        ...
        return {
          reply: 'Wanneer is dit gebeurd? (datum/tijd)',
          rag: { used: false },
        };
      case 'ask_datetime':
        ...
        return {
          reply:
            'Beschrijf de situatie/context (optioneel, anders zeg "skip").',
          rag: { used: false },
        };
      case 'ask_situation':
        ...
        return {
          reply: 'Zijn er bewijzen of getuigen? (beschrijf, of zeg "nee")',
          rag: { used: false },
        };
      case 'ask_evidence':
        ...
        return {
          reply: 'Heeft u nog extra relevante informatie? (typ of zeg "nee")',
          rag: { used: false },
        };
      case 'ask_extra_loop':
        if (/nee|geen/i.test(lower)) {
          ...
          const summary = buildSummary(f);
          return {
            reply: `Samenvatting:\n${summary}\nIs dit correct en wilt u indienen als proces-verbaal? (ja/nee)`,
            rag: { used: false },
          };
        } else {
          f.additional.push(message.trim());
          return {
            reply: 'Nog meer extra info? (typ of zeg "nee")',
            rag: { used: false },
          };
        }
      case 'confirm_summary':
        ...
        if (/nee|niet|fout|aanpassen/.test(lower)) {
          ...
          return {
            reply:
              'Welke veld wilt u wijzigen? (naam, beschrijving, situatie, locatie, datum, dader, bewijs, extra)',
            rag: { used: false },
          };
        }
        return {
          reply: 'Antwoord alstublieft met ja of nee.',
          rag: { used: false },
        };
      case 'edit_select': {
        const map = {
          ...
          datum: 'datetime',
          tijd: 'datetime',
          dader: 'suspectDetails',
          bewijs: 'evidence',
          extra: 'additional',
        };
        let chosen = null;
        for (const k of Object.keys(map))
          if (lower.includes(k)) chosen = map[k];
        if (!chosen)
          return {
            reply:
              'Ik herkende het veld niet. Kies uit: naam, beschrijving, situatie, locatie, datum, dader, bewijs, extra',
            rag: { used: false },
          };
        st.report.pendingEditField = chosen;
        st.report.step = 'edit_value';
        return {
          reply: `Geef nieuwe waarde voor ${chosen}.`,
          rag: { used: false },
        };
      }
      case 'edit_value':
        ...
        return {
          reply: `Aangepaste samenvatting:\n${summary2}\nIs dit nu correct? (ja/nee)`,
          rag: { used: false },
        };
      default:
        return {
          reply:
            'Workflow status onbekend, we beginnen opnieuw. Typ iets om te starten.',
          rag: { used: false },
        };
    }
  }
  ...
  if (st.mode === 'question' || isTrafficQuestion(message) || isQuestion) {
    ...
    if (docs.length) {
      ...
      const hits = docs
        .map(
          (d) =>
            `#${d.id}(emb=${d.embScore.toFixed(3)},lex=${d.lexScore.toFixed(
              3
            )},sum=${d.score.toFixed(3)})`
        )
        .join(', ');
      ...
      for (const d of docs) {
        ...
        console.log(
          `  [Doc ${d.id}] emb=${d.embScore.toFixed(
            3
          )} lex=${d.lexScore.toFixed(3)} sum=${d.score.toFixed(
            3
          )} preview='${preview}'`
        );
      }
    ...
    console.log(
      `[RAG] Query: "${message}" -> geen relevante documenten (fallback antwoord).`
    );
    return { reply: ans, rag: { used: false } };
  }
  if (lower.includes('hallo') || lower.includes('goedendag'))
    return {
      reply: 'Goedendag. Gaat het om een aangifte of een algemene vraag?',
      rag: { used: false },
    };
  return {
    reply: 'Gaat dit over een aangifte die u wilt maken of een algemene vraag?',
    rag: { used: false },
  };
}
...
app.post('/api/chat', async (req, res) => {
  try {
    ...
    if (!sessionId)
      return res.status(400).json({ error: 'sessionId ontbreekt' });
    ...
    conversationContext[sessionId].push({
      role: 'assistant',
      content: result.reply,
    });
    res.json({
      response: result.reply,
      dossierState: sessionState[sessionId],
      rag: result.rag,
    });
  } catch (e) {
    console.error(e);
...
app.post('/api/ingest-pdf', async (req, res) => {
  try {
    ...
    for (const chunk of chunks) {
      ...
      db.run(
        'INSERT INTO verkeersregels (content, embedding) VALUES (?, ?)',
        [chunk, JSON.stringify(emb)],
        (err) => {
...
// ----------------------- DOSSIERS API (admin) -----------------------
// GET list (paginated)
app.get('/api/dossiers', requireAdmin, (req, res) => {
  const limit = Math.min(parseInt(req.query.limit) || 50, 500);
  const offset = Math.max(parseInt(req.query.offset) || 0, 0);
  db.all(
    'SELECT id, datum, beschrijving, samenvatting, prioriteit, status FROM dossiers ORDER BY id DESC LIMIT ? OFFSET ?',
    [limit, offset],
    (err, rows) => {
...
// GET single dossier
app.get('/api/dossiers/:id', requireAdmin, (req, res) => {
  const id = parseInt(req.params.id);
  if (!id) return res.status(400).json({ error: 'Invalid id' });
  db.get('SELECT * FROM dossiers WHERE id = ?', [id], (err, row) => {
    if (err) return res.status(500).json({ error: 'DB error' });
...
// PUT update dossier (partial)
app.put('/api/dossiers/:id', requireAdmin, (req, res) => {
  const id = parseInt(req.params.id);
  if (!id) return res.status(400).json({ error: 'Invalid id' });
  const allowed = [
    'datum',
  ...
  params.push(id);
  const sql = `UPDATE dossiers SET ${updates.join(', ')} WHERE id = ?`;
...
// DELETE dossier
app.delete('/api/dossiers/:id', requireAdmin, (req, res) => {
  const id = parseInt(req.params.id);
  if (!id) return res.status(400).json({ error: 'Invalid id' });
