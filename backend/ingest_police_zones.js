...
function parsePoliceData(rawText) {
  ...
  for (const block of blocks) {
    ...
    const possibleZones = content.match(
      /\b(?:ZP|PZ|POLICE|ZONE|POLIZEIZONE)?\s*[A-ZÀ-ÖØ-Þ\-\s&]{3,}\b/g
    );
    if (possibleZones) {
...
async function run() {
  ...
  await new Promise((resolve) =>
    db.run('DROP TABLE IF EXISTS police_zones', resolve)
  );
  await new Promise((resolve) =>
    db.run(
      `
    ...
    for (const file of files) {
      ...
      for (const rec of records) {
        ...
        if (['5267', '5271', '5277', '5279'].includes(rec.id)) {
          ...
          console.log(
            `   Gemeentes:      ${JSON.stringify(rec.municipalities)}`
          );
